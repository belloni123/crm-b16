  ...
  const mailHtml = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 30px 20px; background-color: #050505; border-radius: 16px; border: 1px solid rgba(212, 168, 67, 0.1); color: #ffffff; text-align: center;">
      <div style="margin-bottom: 24px;">
        <h1 style="color: #d4a843; margin: 0; font-size: 24px; font-weight: 800; letter-spacing: -0.02em; font-family: 'Outfit', sans-serif;">CRM B16</h1>
      </div>
      ...
      <p style="font-size: 15px; line-height: 1.6; color: #f2f9ed; margin-bottom: 30px; text-align: left;">
        Você solicitou a redefinição de senha para a sua conta no CRM b16. Para criar uma nova senha, clique no botão abaixo:
      </p>
      
      <div style="margin-bottom: 36px; text-align: center;">
        <a href="${resetUrl}" style="display: inline-block; background-color: #d4a843; color: #0a0a0a; font-weight: bold; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-size: 15px; box-shadow: 0 0 15px rgba(212, 168, 67, 0.3); transition: all 0.2s;">
          Redefinir Senha
        </a>
      </div>
      ...
      <div style="border-top: 1px solid rgba(212, 168, 67, 0.08); padding-top: 20px; font-size: 12px; color: #4e5c46; text-align: left; word-break: break-all;">
        Se o botão acima não funcionar, copie e cole o link a seguir no seu navegador:<br>
        <a href="${resetUrl}" style="color: #d4a843; text-decoration: underline; display: block; margin-top: 5px;">${resetUrl}</a>
      </div>
    </div>
  `;

  const mailFrom = process.env.MAIL_FROM || 'CRM b16 <noreply@crmb16.com.br>';
...
export async function sendProjectInvitationEmail(email: string, userName: string | null, projectName: string, projectUrl: string) {
  ...
  const mailHtml = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 30px 20px; background-color: #050505; border-radius: 16px; border: 1px solid rgba(212, 168, 67, 0.1); color: #ffffff; text-align: center;">
      ...
      <p style="font-size: 15px; line-height: 1.6; color: #f2f9ed; margin-bottom: 30px; text-align: left;">
        Você foi convidado(a) para colaborar no projeto comercial <strong>${projectName}</strong> no CRM b16.
      </p>
      
      <div style="margin-bottom: 36px; text-align: center;">
        <a href="${projectUrl}" style="display: inline-block; background-color: #d4a843; color: #0a0a0a; font-weight: bold; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-size: 15px; box-shadow: 0 0 15px rgba(212, 168, 67, 0.3); transition: all 0.2s;">
          Acessar Projeto
        </a>
      </div>
      ...
      <div style="border-top: 1px solid rgba(212, 168, 67, 0.08); padding-top: 20px; font-size: 12px; color: #4e5c46; text-align: left; word-break: break-all;">
        Se o botão acima não funcionar, copie e cole o link a seguir no seu navegador:<br>
        <a href="${projectUrl}" style="color: #d4a843; text-decoration: underline; display: block; margin-top: 5px;">${projectUrl}</a>
      </div>
    </div>
  `;
