...
export function createFoundationQueue(name: QueueName) {
  return new Queue(name, { prefix: queuePrefix(), connection: redisConnectionOptions(), defaultJobOptions: { attempts: 3, backoff: { type: "exponential", delay: 1000 }, removeOnComplete: 100 } });
}
