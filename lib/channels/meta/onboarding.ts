...
export async function createMetaOnboardingSession(projectId: string, userId: string) {
  ...
  const session = await prisma.metaOnboardingSession.create({ data: { projectId, userId, stateHash: hash(state), nonceHash: hash(nonce), expiresAt: new Date(Date.now() + 10 * 60_000) } });
  return { sessionId: session.id, state, nonce, appId: process.env.META_APP_ID!, configId: process.env.META_CONFIG_ID!, graphVersion: process.env.META_GRAPH_API_VERSION || "v24.0" };
}
...
export async function completeMetaOnboarding(input: CompleteInput) {
  ...
  try {
    ...
    if (wabaTargets.size && !wabaTargets.has(input.wabaId)) throw new Error("META_WABA_NOT_GRANTED");

    const graph = new MetaGraphClient(exchanged.accessToken);
    const waba = await graph.get<{ id: string; name?: string; account_review_status?: string }>(`/${encodeURIComponent(input.wabaId)}?fields=id,name,account_review_status`);
    ...
    const data = {
      externalWabaId: input.wabaId, externalPhoneNumberId: input.phoneNumberId,
